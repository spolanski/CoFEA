...
for result in result_file_names:
    ...
    elif 'codeaster' in key:
        x, y = df['Points:1'].to_list(), df["resnonl_SIEQ_NOEU:0"].to_list()
    else:
        print(key)
        raise ValueError
...
lin_results = lin_results.drop(['Shape'], axis=1)
print(lin_results.to_markdown())
lin_results.to_csv('quad_shape.csv')
