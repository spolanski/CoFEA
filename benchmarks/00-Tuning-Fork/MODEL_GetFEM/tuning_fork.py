...
###############################################################################
# Plot modes using PyVista.
#

for i in range(12):

    f0 = np.loadtxt("freqs" + "{:0=3}".format(i) + ".txt")
    m = pv.read("vecs" + "{:0=3}".format(i) + ".vtk")

    p = pv.Plotter(shape=(2, 3))

    p.subplot(0, 0)
    p.add_mesh(m, show_scalar_bar=False)
    p.camera_position = "xy"
    p.show_axes()

    p.subplot(0, 1)
    w1 = m.warp_by_vector("Mode_1", factor=0.01)
    p.add_mesh(w1, show_scalar_bar=False)
    p.camera_position = "xy"
    p.add_text("Mode 1: " + "{:.2f}".format(f0[0]) + "Hz")
    p.show_axes()

    p.subplot(0, 2)
    w2 = m.warp_by_vector("Mode_2", factor=0.01)
    p.add_mesh(w2, show_scalar_bar=False)
    p.camera_position = "zy"
    p.add_text("Mode 2: " + "{:.2f}".format(f0[1]) + "Hz")
    p.show_axes()

    p.subplot(1, 0)
    w3 = m.warp_by_vector("Mode_3", factor=0.01)
    p.add_mesh(w3, show_scalar_bar=False)
    p.camera_position = "xy"
    p.add_text("Mode 3: " + "{:.2f}".format(f0[2]) + "Hz")
    p.show_axes()

    p.subplot(1, 1)
    w4 = m.warp_by_vector("Mode_4", factor=0.01)
    p.add_mesh(w4, show_scalar_bar=False)
    p.camera_position = "zy"
    p.add_text("Mode 4: " + "{:.2f}".format(f0[3]) + "Hz")
    p.show_axes()

    p.subplot(1, 2)
    w5 = m.warp_by_vector("Mode_5", factor=0.01)
    p.add_mesh(w5, show_scalar_bar=False)
    p.camera_position = "xy"
    p.add_text("Mode 5: " + "{:.2f}".format(f0[4]) + "Hz")
    p.show_axes()

    p.show(screenshot="modes" + "{:0=3}".format(i) + ".png")

print("Time for plot results", time.process_time() - t)
t = time.process_time()
